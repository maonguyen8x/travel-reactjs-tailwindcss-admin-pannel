...
const FormAddBackground = (props: EnhancedProps) => {
      ...
      <>
        ...
        <div className="flex flex-row justify-between items-center space-x-10 mx-4">
          {DATA?.map((items: any, index: number) => (
            <div
              className={`w-8 h-8 ring-1 rounded-full cursor-pointer bg-${items?.color}`}
              key={index}
  ...
  const [imagePreview, setImagePreview]: any = useState(
    backgroundPostDetail?.backgroundPost
  );

  useLayoutEffect(() => {
    setImagePreview(backgroundPostDetail?.backgroundPost);
    setValues({
      ...values,
      data: backgroundPostDetail,
    });
  }, [backgroundPostDetail?.backgroundPost]);
  ...
  const onRemoveImage = () => {
    ...
    const mediaCache = [backgroundPostDetail?.backgroundPost];
    mediaCache.pop();
    setImagePreview(mediaCache);
  };
        ...
        <div>
          ...
          <div className="relative bg-gray-200 flex items-center">
            ...
            <span className="absolute left-40 bg-transparent text-default ">
              {values?.name || imagePreview?.fileName}
            </span>
          </div>
          <div>
            {(values?.url || !isEmpty(imagePreview)) && (
              <div className="mt-8 relative">
                <SquareImage
                  className=""
                  src={values?.url || imagePreview?.url}
                />
                <i
        ...
        <div>
          ...
          <div className="relative bg-gray-200 p-3">
            ...
            <span className="ml-28 bg-transparent text-default ">
              {values?.color || backgroundPostDetail?.color}
            </span>
          </div>
